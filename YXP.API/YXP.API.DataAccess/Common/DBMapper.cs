        ...
        private static void InitDB(string connGroupName, IEnumerable<ConnectionItem> list, int state, ref List<Database> db)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        var cfg = new DapperExtensionsConfiguration(typeof(AutoClassMapper<>), new List<Assembly>(), item2.dBase);
                        var tdb = new Database(item2.Connection, new SqlGeneratorImpl(cfg));
                        db.Add(tdb);
                    }
                }
